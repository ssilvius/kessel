...
- `conversation_quest_refs` table -- the connective tissue between NPCs and quests. NODE prototype files (`/resources/systemgenerated/prototypes/<num>.node`) hold each `cnv.*` conversation's playback data. The PROT header carries the cnv FQN; the body contains CF GUID refs to qst.* objects representing quests that conversation grants or affects. ~23% of NODE files (2,494 of 10,735) carry such refs, covering 1,056 of 1,315 quests (80%). Enables giver-NPC inference by joining cnv FQN -> npc FQN via shared name segment.
